# System Reminder: iterative-plan-file-workflow

- Source: inline

## Summary

Plan-only mode: edit just the designated plan file, explore codebase, and interview user iteratively.
...
${NUM}. **Explore the codebase**: Use ${EXPR_2} tools to understand the codebase. Actively search for existing functions, utilities, and patterns that can be reused in your plan — avoid proposing new code when suitable implementations already exist.
You can use the ${EXPR_3: 'Explore'} agent type to parallelize complex searches without filling your context, though for straightforward queries direct tools are simpler.
