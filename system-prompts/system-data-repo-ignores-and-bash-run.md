# System Data Block: repo-ignores-and-bash-run

- Source: inline

## Summary

Output listing repo paths like git and editor folders plus a background bash run and args.

## Placeholder Hints (source-backed)

| Expression | Hint | Reference |
| --- | --- | --- |
| `EXPR_1` | None | None |
| `EXPR_2` | None | None |
| `EXPR_3` | None | None |
| `EXPR_4` | None | None |
| `EXPR_5` | None | None |
| `EXPR_6` | Task | None |
| `EXPR_7` | Bash | None |
| `EXPR_8` | Glob | None |
| `EXPR_9` | Grep | None |
| `EXPR_10` | ExitPlanMode | None |
| `EXPR_11` | Read | None |
| `EXPR_12` | Edit | None |
| `EXPR_13` | MultiEdit | None |
| `EXPR_14` | Write | None |
| `EXPR_15` | NotebookEdit | None |
| `EXPR_16` | WebFetch | None |
| `EXPR_17` | WebSearch | None |
| `EXPR_18` | BashOutput | None |
| `EXPR_19` | KillShell | None |
| `EXPR_20` | None | None |
| `EXPR_21` | None | None |
| `EXPR_22` | None | None |
| `EXPR_23` | stream-json | None |
| `EXPR_24` | None | None |

# Raw Prompt Text
.git

.vscode

.idea

${EXPR_1}

${EXPR_2}

${EXPR_3}

${EXPR_4}

${EXPR_5}

${EXPR_6: 'Task'}

${EXPR_7: 'Bash'}

${EXPR_8: 'Glob'}

${EXPR_9: 'Grep'}

${EXPR_10: 'ExitPlanMode'}

${EXPR_11: 'Read'}

${EXPR_12: 'Edit'}

${EXPR_13: 'MultiEdit'}

${EXPR_14: 'Write'}

${EXPR_15: 'NotebookEdit'}

${EXPR_16: 'WebFetch'}

${EXPR_17: 'WebSearch'}

${EXPR_18: 'BashOutput'}

${EXPR_19: 'KillShell'}

Background Bash ${EXPR_20}

(command: ${EXPR_21})

(status: ${EXPR_22})

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

${NUM}

args

${EXPR_23: 'stream-json'}

null

${EXPR_24}

${EXPR_25}

${EXPR_26}
