# Claude Code Metadata x.y.z

## Embedded Metadata

- Package URL: @anthropic-ai/claude-code
- README URL: https://code.claude.com/docs/en/overview

## Bundle

- Entry bytes: 11216990 bytes
- Entry lines: 5628
- Pretty printer: esbuild
- Pretty printer version: 0.27.2
- Pretty bytes: 14444844 bytes
- Pretty lines: 272208

## Inventory

- File count: 20
- Size bytes: 71106707

### Extension Counts

- `.exe`: 1 files, 5407744 bytes
- `.js`: 1 files, 11216990 bytes
- `.json`: 1 files, 1199 bytes
- `.lock`: 1 files, 551 bytes
- `.md`: 2 files, 2184 bytes
- `.node`: 5 files, 28944544 bytes
- `.ts`: 1 files, 66576 bytes
- `.wasm`: 3 files, 4064873 bytes

### Asset Sizes

- `.exe`: 5407744 bytes
- `.node`: 28944544 bytes
- `.wasm`: 4064873 bytes

### Files

- `bun.lock`
- `cli.js`
- `LICENSE.md`
- `package.json`
- `README.md`
- `resvg.wasm`
- `sdk-tools.d.ts`
- `tree-sitter-bash.wasm`
- `tree-sitter.wasm`
- `vendor/ripgrep/arm64-darwin/rg`
- `vendor/ripgrep/arm64-darwin/ripgrep.node`
- `vendor/ripgrep/arm64-linux/rg`
- `vendor/ripgrep/arm64-linux/ripgrep.node`
- `vendor/ripgrep/COPYING`
- `vendor/ripgrep/x64-darwin/rg`
- `vendor/ripgrep/x64-darwin/ripgrep.node`
- `vendor/ripgrep/x64-linux/rg`
- `vendor/ripgrep/x64-linux/ripgrep.node`
- `vendor/ripgrep/x64-win32/rg.exe`
- `vendor/ripgrep/x64-win32/ripgrep.node`

## Snapshot Scan Provenance

- Snapshot generated at: 2026-02-18 17:47:04 UTC
- Node runtime: `v22.22.0`
- Config hash: `5e18cf71066d3a7acbebcd16a03121ee7a19e5a3d9aba70b766188afe31b8e99`
- Embedded build timestamp: 2026-01-15 23:59:03 UTC
- Relative to embedded build timestamp: 33d 17h 48m 1s (2026-01-15 23:59:03 UTC → 2026-02-18 17:47:04 UTC)

## Prompt Token Distribution

- Total prompt tokens: 331,444
- Token p95 / p99: 1,099 / 2,486

### By Kind

- `system-data`: 258,632 tokens (78.0%)
- `system`: 42,621 tokens (12.9%)
- `system-reminder`: 11,429 tokens (3.4%)
- `tool`: 11,408 tokens (3.4%)
- `skill`: 4,267 tokens (1.3%)
- `agent`: 3,087 tokens (0.9%)
